...
├── styles.css                    # スタイルシート
├── docs/                         # ドキュメント
│   ├── funcspec-202608.md        # 機能仕様書（v6.4・本書）
│   ├── UsersGuide-202608.md      # ユーザーガイド（v6.4）
│   ├── funcspec-202607.md        # 機能仕様書（v6.3・旧版）
│   └── UsersGuide-202607.md      # ユーザーガイド（v6.3・旧版）
├── icons/                        # SVGアイコン
└── js/                           # JavaScriptモジュール
    ...
    │   ├── SpotUIManager.js      # スポットUI管理（領域指定削除・v6.4 新規）
...
#### 3.5.2 領域を指定したスポット削除（v6.4 新規）
スポット編集パネルのスポット数欄の上に、領域を指定して削除する2つのボタンを配置する。
MapEditorの「重複スポット抽出」と同様に、ボタンで領域選択モードに入り、画像上をドラッグして囲んだ矩形範囲を処理対象とする。

| ボタン | 処理内容 |
|---|---|
| **重複スポットの削除** | 範囲内で「同名かつ30px以内」のスポットを重複とみなし、各グループで1個だけ残して削除 |
| **選択領域内のスポット削除** | 範囲内のスポットをすべて削除 |

**操作フロー**:
1. ボタンをクリック → 領域選択モード開始（ボタンが赤く反転、案内メッセージを表示）
2. 画像上をドラッグ → 薄いピンクの矩形で範囲を表示（ルート中間点の範囲削除と同じ描画）
3. マウスを離す → 削除対象を確認ダイアログで提示（対象名の一覧、11件目以降は「…ほかN件」に省略）
4. OK → 削除実行・スポット数と入力ボックスを更新／キャンセル → 何も削除しない
5. ボタンを再クリック、または他の編集モードへ切り替えで領域選択モードを解除

**仕様上の注意**:
- 重複判定のしきい値は `DUPLICATE_SPOT_DISTANCE = 30`（キャンバス座標のピクセル、`SpotManager.js` で定義）。同名でも離れているスポットは削除対象外
- 名称が未入力のスポットは重複判定の対象外（「選択領域内のスポット削除」では削除対象に含む）
- 削除対象がルートの開始・終了ポイントとして使われている場合は、確認ダイアログに該当スポット名を警告表示
- ドラッグ距離が3px未満の場合は誤クリックとみなし、何も削除しない（新規スポットも追加しない）
- 領域選択モード中はスポット名ポップアップを `pointer-events: none` にし、ポップアップ上を通過してもドラッグが途切れないようにする
- 削除はローカルデータのみに反映（Firestoreへは「保存」ボタンで同期）

**主なAPI**:
- `SpotManager.findSpotsInRectangle(x1, y1, x2, y2)`: 矩形範囲内のスポットを検索
- `SpotManager.findDuplicateSpotsInRectangle(x1, y1, x2, y2, distanceLimit)`: 重複グループ `{name, keepIndex, removeIndexes}` を検索
- `SpotManager.removeSpots(indices)`: 複数スポットの一括削除（通知は1回のみ）
...
1. **領域を指定したスポット削除機能の追加**（3.5.2節）
   - スポット編集パネルに「重複スポットの削除」「選択領域内のスポット削除」ボタンを追加（スポット数欄の上）
   - ボタンで領域選択モードに入り、画像上をドラッグして囲んだ矩形範囲を処理対象とする
   - 重複判定は「同名かつ30px以内」（`DUPLICATE_SPOT_DISTANCE`）で、各グループから1個だけ残す
   - 削除前に確認ダイアログを表示し、ルートの開始・終了ポイントに使われているスポットは警告
   - ボタンは縦2段配置とし、コントロールパネルの幅は他の編集モードと同じまま維持
   - 新規モジュール: `js/ui/SpotUIManager.js`
   - 新規API: `SpotManager.findSpotsInRectangle()` / `findDuplicateSpotsInRectangle()` / `removeSpots()`

### v6.3 (2026年7月版)
**主な変更点**:

1. **ルート経路の描画機能の追加**（3.4.4節）
   - 「ルート経路を描画」チェックボックス（デフォルト: オフ）を追加
   - 選択中ルートの開始ポイント→中間点→終了ポイントをオレンジ色の折れ線で表示

2. **中間点の経路最適化機能の追加**（3.4.5節）
